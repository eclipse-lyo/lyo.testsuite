...
public class ServiceProviderCatalogTests {
	...
	public static Collection<Object[]> getReferencedCatalogUrls(String base) throws IOException, ParserConfigurationException, SAXException, XPathException
	{
		Properties setupProps = SetupProperties.setup(null);
		String userId = setupProps.getProperty("userId");
		String pw =  setupProps.getProperty("pw");
		
		HttpResponse resp = OSLCUtils.getResponseFromUrl(base, base, new UsernamePasswordCredentials(userId, pw), OSLCConstants.CT_DISC_CAT_XML);
	    ...
	    NodeList spcs = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:entry/oslc_disc:ServiceProviderCatalog/@rdf:about", baseDoc,
	...
	public void catalogRootIsServiceProviderCatalog() throws XPathException 
	{
        ...
        Node rootNode = (Node) OSLCUtils.getXPath().evaluate("/oslc_disc:ServiceProviderCatalog", doc, XPathConstants.NODE);
	...
	{
		//Make sure that we our root element has an rdf:about that points the same server provider catalog
        Node aboutRoot = (Node) OSLCUtils.getXPath().evaluate("/oslc_disc:ServiceProviderCatalog/@rdf:about", doc,
        		XPathConstants.NODE);
        assertNotNull(aboutRoot);
        HttpResponse resp = OSLCUtils.getResponseFromUrl(baseUrl, aboutRoot.getNodeValue(), basicCreds, "*/*");
	...
	{
		//Check root
		Node rootCatalogTitle = (Node) OSLCUtils.getXPath().evaluate("/oslc_disc:ServiceProviderCatalog/dc:title", doc,
				XPathConstants.NODE);
		assertNotNull(rootCatalogTitle);
		assertFalse(rootCatalogTitle.getTextContent().isEmpty());
		NodeList titleSub = (NodeList)OSLCUtils.getXPath().evaluate("/oslc_disc:ServiceProviderCatalog/dc:title/*", doc,
				XPathConstants.NODESET);
		assertTrue(titleSub.getLength() == 0);
		
        //Get all entries, parse out which have embedded catalogs and check the titles
		NodeList catalogs= (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:entry/*", 
				doc, XPathConstants.NODESET);
		for (int i = 0; i < catalogs.getLength(); i++)
		{
			Node catalog = (Node) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) + 
					"]/oslc_disc:ServiceProviderCatalog", doc, XPathConstants.NODE);
			//This entry has a catalog, check that it has a title
			if (catalog != null)
			{
				Node cTitle = (Node) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) +
						"]/oslc_disc:ServiceProviderCatalog/dc:title", doc, XPathConstants.NODE);
				assertNotNull(cTitle);
				//Make sure the child isn't empty
				assertFalse(cTitle.getTextContent().isEmpty());
				Node child = (Node) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) +
						"]/oslc_disc:ServiceProviderCatalog/dc:title/*", doc, XPathConstants.NODE);
	...
	public void serviceProvidersHaveValidTitles() throws XPathException 
	{
		...
		NodeList entries= (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:entry/*", 
				doc, XPathConstants.NODESET);
		for (int i = 0; i < entries.getLength(); i++)
		{
			Node provider = (Node) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) + 
					"]/oslc_disc:ServiceProvider", doc, XPathConstants.NODE);
			//This entry has a catalog, check that it has a title
			if (provider != null)
			{
				Node pTitle = (Node) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) +
						"]/oslc_disc:ServiceProvider/dc:title", doc, XPathConstants.NODE);
	...
	{
        //Get all ServiceProviderCatalog elements and their rdf:about attributes, make sure that each catalog has
		//an rdf:about attribute
		NodeList catalogAbouts = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:ServiceProviderCatalog/@rdf:about", doc,
				XPathConstants.NODESET);
		NodeList catalogs = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:ServiceProviderCatalog", doc, 
				XPathConstants.NODESET);
	...
	public void entryElementsHaveSingleServiceProviderOrCatalog() throws XPathException 
	{
		...
		for (int i = 0; i < entries.getLength(); i++)
		{
			...
			NodeList spc = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) + 
					"]/oslc_disc:ServiceProviderCatalog", doc, XPathConstants.NODESET);
			if (spc.getLength() == 1)
			{
				hasServiceOrCatalog = true;
			}
			NodeList sp = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) +
					"]/oslc_disc:ServiceProvider", doc, XPathConstants.NODESET);
			if (sp.getLength() == 1)
			{
	...
	public void noInternalServiceProviderCatalogsHaveEntryElements() throws XPathException
	{
		...
		NodeList internEntry = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:entry/oslc_disc:ServiceProviderCatalog" +
				"/oslc_disc:entry", doc, XPathConstants.NODESET);
	...
	public void serviceProviderElementsHaveServicesChildElement() throws XPathException
	{
		...
		for (int i = 0; i < entries.getLength(); i++)
		{
			Node sp = (Node) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) + "]/oslc_disc:ServiceProvider",
					doc, XPathConstants.NODE);
			//This entry has a ServiceProvider and not a catalog
			if (sp != null)
			{
				//Verify the ServiceProvider has a child element services
				Node services = (Node) OSLCUtils.getXPath().evaluate("//oslc_disc:entry[" + (i + 1) + 
						"]/oslc_disc:ServiceProvider/oslc_disc:services", doc, XPathConstants.NODE);
	...
	public void servicesChildElementHasValidResourceAttribute() throws XPathException, IOException
	{
		...
		NodeList services = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:services", doc, XPathConstants.NODESET);
		//Get all resource attributes from services
		NodeList resources = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:services/@rdf:resource", 
				doc, XPathConstants.NODESET);
	...
	public void detailsElementsHaveValidResourceAttribute() throws IOException, XPathException
	{
		...
		NodeList resources = (NodeList) OSLCUtils.getXPath().evaluate("//oslc_disc:details/@rdf:resource", doc, 
				XPathConstants.NODESET);
